...
evento = 1; % Selecciono cuales de los eventos en donde la LOS cambia de signo (o sea hubo un evento).
...
MuestrasLOSInterp = interp1(tEvento,MuestrasLOSEvento, tInterp, 'linear'); % LOS para esa resolución
MuestrasDopplerInterp = interp1(tEvento,MuestrasDopplerEvento,tInterp,'linear'); % Doppler para esa resolución 
...
coefAMP = polyfit(h_LOS_datos, amplitude_LEO, 9); % Ajustamos la amplitud
Amplitude_AJ = polyval(coefAMP, MuestrasLOSInterp);
...
sRO = ampRO.*exp(1j*(2*pi*(fL1 + fD_GEOM).*tInterp + phaseRO));
...
figure;
subplot(3,1,1)
plot((1:length(fD))*resolucion,fD,'LineWidth',1);
ylabel('$f_{Doppler}$','Interpreter','latex')
subplot(3,1,2)
plot((1:length(hminLOS))*resolucion,hminLOS,'LineWidth',1)
ylabel('$h_{LOS}$','Interpreter','latex')
subplot(3,1,3)
plot(1:length(hminLOS)-1,diff(hminLOS),'LineWidth',1)
